...
void Style(
    ...
    LCMSAxis axis, int centIdx, int ktIdx
) {
    // General
    {
...
TH1D* ProjectRatio(
    ...
    Style(r, fmt, axis, centIdx, ktIdx);
    return r;
}
...
TH1D* RatioProject(
    ...
    Style(ratio, fmt, axis, centIdx, ktIdx);
    return ratio;
}
...
void do_CF_ratios(TFile* fCF3D, TFile* fRatioProj, TFile* fProjRatio) {
    ...
    for (int ktIdx = 0; ktIdx < ktSize; ktIdx++) {
        ...
        for (auto axis : {LCMSAxis::Out, LCMSAxis::Side, LCMSAxis::Long}) {
            // Отношение проекций
            {
                TH1D* h = RatioProject(*neg, *pos, centIdx, ktIdx, axis);
                fRatioProj->cd();
                h->Write();
            }
            ...
            // Проекция отношения
            {
                TH1D* h = ProjectRatio(*ratio, centIdx, ktIdx, axis);
                fProjRatio->cd();
                h->Write();
            }
        }
